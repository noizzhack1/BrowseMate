...
class MCPClient {
  ...
  async sendSSERequest(serverConfig, method, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Set appropriate Accept header based on transport type
      if (serverConfig.type === 'http') {
        // HTTP (Streamable-HTTP) transport REQUIRES both content types
        // FastMCP explicitly checks for this: "Client must accept both application/json and text/event-stream"
        headers['Accept'] = 'text/event-stream, application/json';
      } else if (serverConfig.type === 'sse') {
        // SSE transport may return text/event-stream format
        headers['Accept'] = 'text/event-stream, application/json';
      }
  ...
  /**
   * Send a JSON-RPC notification to an MCP server (fire-and-forget, no response expected)
   * Notifications differ from requests in that they have NO id field per JSON-RPC 2.0 spec
   * @param {Object} serverConfig - Server configuration
   * @param {string} method - JSON-RPC notification method name
   * @param {Object} params - Method parameters
   * @returns {Promise<void>} - Resolves when notification is sent (does not wait for response)
   */
  async sendNotification(serverConfig, method, params = {}) {
    // Log the notification being sent (no ID for notifications)
    Logger.info(`[MCPClient.sendNotification] Sending notification ${method} to ${serverConfig.name}`);

    // Build the JSON-RPC notification object (NO id field - this is the key difference from requests)
    // Per JSON-RPC 2.0: "A Notification is a Request object without an 'id' member"
    const notification = {
      jsonrpc: '2.0',
      method: method,
      params: params
      // NOTE: Deliberately NO 'id' field - this makes it a notification, not a request
    };
    ...
    if (existingSessionId) {
      // Include the session ID from previous initialize response
      headers['Mcp-Session-Id'] = existingSessionId;
      Logger.debug(`[MCPClient.sendNotification] Including session ID: ${existingSessionId}`);
    }

    // Determine the POST endpoint (same logic as sendSSERequest)
    let postUrl = serverConfig.url;
    ...
    Logger.debug(`[MCPClient.sendNotification] POST URL: ${postUrl}`);

    try {
      // Send the notification via POST (fire-and-forget)
      ...
      // Log the response status (we don't parse the body for notifications)
      Logger.debug(`[MCPClient.sendNotification] Response status: ${response.status}`);

      // For notifications, we don't expect a meaningful response
      // A 200/202/204 status is fine, errors are logged but not thrown
      if (!response.ok) {
        Logger.warn(`[MCPClient.sendNotification] Notification ${method} returned status ${response.status}`);
      } else {
        Logger.info(`[MCPClient.sendNotification] Notification ${method} sent successfully`);
      }
    } catch (error) {
      // Log network errors but don't throw - notifications are best-effort
      Logger.warn(`[MCPClient.sendNotification] Failed to send notification ${method}:`, error);
    }
  }
  ...
  async initializeServer(serverConfig) {
    ...
    Logger.info(`[MCPClient.initializeServer] Transport type: ${serverConfig.type}`);
    ...
    try {
      ...
      if (serverConfig.type === 'websocket') {
        ...
        // Use HTTP transport (includes both 'http' and 'sse' types)
        // Both use POST-based sendSSERequest method
      ...
      // Send initialized notification (no response expected, but MCP protocol requires it)
      // Use sendNotification() which sends WITHOUT an 'id' field (proper JSON-RPC notification)
      // This fixes the "Failed to validate request" error from FastMCP servers
      await this.sendNotification(serverConfig, 'notifications/initialized', {});
  ...
  async discoverTools(serverConfig) {
    ...
    Logger.info(`[MCPClient.discoverTools] Transport type: ${serverConfig.type}`);
  ...
  async callTool(serverConfig, toolName, args = {}) {
    ...
    Logger.info(`[MCPClient.callTool] Transport type: ${serverConfig.type}`);
    Logger.debug(`[MCPClient.callTool] Arguments:`, args);

    try {
      // IMPORTANT: Ensure we have a valid session before calling tools
      // FastMCP's streamable-http transport requires an initialized session
      // Check if we have a stored session ID for this server
      const existingSessionId = this.sessionIds.get(serverConfig.url);
      if (!existingSessionId) {
        // No session exists - initialize the server first to create one
        Logger.info(`[MCPClient.callTool] No existing session for ${serverConfig.name}, initializing first...`);
        await this.initializeServer(serverConfig);
        Logger.info(`[MCPClient.callTool] Server initialized, session created`);
      } else {
        Logger.debug(`[MCPClient.callTool] Using existing session: ${existingSessionId}`);
      }
      ...
      Logger.info(`[MCPClient.callTool] Tool ${toolName} returned raw result:`, JSON.stringify(result, null, 2));
      Logger.info(`[MCPClient.callTool] Result type:`, typeof result);
      Logger.info(`[MCPClient.callTool] Result keys:`, result ? Object.keys(result) : 'null');
      ...
      if (result && result.content && Array.isArray(result.content) && result.content.length > 0) {
        ...
        Logger.debug(`[MCPClient.callTool] First content item:`, firstContent);
        if (firstContent && firstContent.type === 'text' && firstContent.text !== undefined) {
          ...
          Logger.info(`[MCPClient.callTool] ✅ Extracted text content: ${displayValue}`);
        }
      }
      ...
      if (displayValue === null && result && result.structuredContent) {
        Logger.debug(`[MCPClient.callTool] Found structuredContent:`, result.structuredContent);
        ...
        if (result.structuredContent.result !== undefined) {
          ...
          Logger.info(`[MCPClient.callTool] ✅ Extracted structured content result: ${displayValue}`);
        } else {
          // Otherwise stringify the entire structuredContent
          extractedValue = result.structuredContent;
          displayValue = JSON.stringify(extractedValue);
          Logger.info(`[MCPClient.callTool] ✅ Extracted full structured content: ${displayValue}`);
        }
      }

      // FastMCP might return the result directly (not wrapped) - check if result is a primitive or simple object
      if (displayValue === null && result !== undefined && result !== null) {
        // Check if result is already a simple value (string, number, boolean)
        if (typeof result !== 'object') {
          extractedValue = result;
          displayValue = String(result);
          Logger.info(`[MCPClient.callTool] ✅ Extracted direct primitive value: ${displayValue}`);
        } else if (Array.isArray(result)) {
          // Result is an array
          extractedValue = result;
          displayValue = JSON.stringify(result);
          Logger.info(`[MCPClient.callTool] ✅ Extracted array result: ${displayValue}`);
        } else {
          // Result is an object - try to find a value property or stringify it
          if (result.value !== undefined) {
            extractedValue = result.value;
            displayValue = JSON.stringify(result.value);
            Logger.info(`[MCPClient.callTool] ✅ Extracted result.value: ${displayValue}`);
          } else {
            // Stringify the entire object as fallback
            extractedValue = result;
            displayValue = JSON.stringify(result);
            Logger.info(`[MCPClient.callTool] ⚠️ Using fallback - stringified entire result: ${displayValue}`);
          }
        }
      }

      // If we still don't have a value, log a warning but still return success
      if (displayValue === null) {
        Logger.warn(`[MCPClient.callTool] ⚠️ Could not extract result value, result was:`, result);
        extractedValue = result;
        displayValue = result !== undefined ? String(result) : 'No result value extracted';
      }
      ...
      Logger.error(`[MCPClient.callTool] ❌ Failed to call ${toolName} on ${serverConfig.name}`);
      Logger.error(`[MCPClient.callTool] Error message:`, error.message);
      Logger.error(`[MCPClient.callTool] Error stack:`, error.stack);
      Logger.error(`[MCPClient.callTool] Error name:`, error.name);
      if (error.response) {
        Logger.error(`[MCPClient.callTool] Error response:`, error.response);
      }
      // Return error result with detailed message
      return {
        success: false,
        error: error.message || 'Unknown error',
        message: `MCP tool ${toolName} failed: ${error.message || 'Unknown error'}`,
        errorDetails: {
          name: error.name,
  ...
  async executeMCPTool(prefixedName, args = {}) {
    // Log execution start with full details
    Logger.info(`[MCPClient.executeMCPTool] 🚀 Executing MCP tool: ${prefixedName}`);
    Logger.info(`[MCPClient.executeMCPTool] Arguments:`, JSON.stringify(args, null, 2));

    // Parse the tool name to extract server ID and original name
    const { serverId, toolName } = this.parseMCPToolName(prefixedName);
    Logger.info(`[MCPClient.executeMCPTool] Parsed - serverId: ${serverId}, toolName: ${toolName}`);

    if (!serverId || !toolName) {
      Logger.error(`[MCPClient.executeMCPTool] ❌ Invalid MCP tool name format: ${prefixedName}`);
      return {
        success: false,
    ...
    const enabledServers = await this.getEnabledServers();
    Logger.info(`[MCPClient.executeMCPTool] Found ${enabledServers.length} enabled servers`);
    const serverConfig = enabledServers.find(s => s.id === serverId);

    if (!serverConfig) {
      Logger.error(`[MCPClient.executeMCPTool] ❌ MCP server not found: ${serverId}`);
      Logger.error(`[MCPClient.executeMCPTool] Available server IDs:`, enabledServers.map(s => s.id));
      return {
        success: false,
    ...
    Logger.info(`[MCPClient.executeMCPTool] ✅ Found server config: ${serverConfig.name} (${serverConfig.url})`);

    // Call the tool on the server
    try {
      const result = await this.callTool(serverConfig, toolName, args);
      Logger.info(`[MCPClient.executeMCPTool] ✅ Tool execution completed. Success: ${result.success}`);
      Logger.info(`[MCPClient.executeMCPTool] Result message: ${result.message}`);
      return result;
    } catch (error) {
      Logger.error(`[MCPClient.executeMCPTool] ❌ Exception during tool execution:`, error);
      return {
        success: false,
        error: error.message || 'Unknown error',
        message: `MCP tool execution threw exception: ${error.message || 'Unknown error'}`
      };
    }
  }

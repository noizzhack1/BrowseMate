...
export async function executeActionsSequentially(actions, onProgress = null, originalUserPrompt = '', enableAdaptivePlanning = true, abortSignal = null) {
  ...
  if (completedCount > 0 && originalUserPrompt) {
    ...
    try {
      ...
      if (abortSignal && abortSignal.aborted) {
        ...
        // Override success status if goal was not achieved
        if (!goalAchieved) {
  ...
  } else if (success && !goalAchieved && completedCount > 0) {
    // Actions succeeded but goal not achieved
    message = `⚠️ Actions completed but goal not achieved\n\n`;
    if (verificationMessage) {
      message += `${verificationMessage}\n\n`;
    }
    message += `The actions were executed successfully, but the final result doesn't match what you asked for. You may need to try a different approach.`;

...
function isResearchQuestion(prompt) {
  ...
  const questionPatterns = [
    ...
    /\b(summarize|summarise|summerize|summerise|summary|summaries|describe|explain|tell me about|give me|show me|show|display)\b.*\b(file|content|info|information|details|summary|summaries|overview|brief)\b/i, // Summarize/describe file content (with noun and verb forms)
    /\b(read|find|look for|search for)\b.*\b(and|then)\b.*\b(summarize|summarise|summerize|summerise|summary|summaries|describe|explain|tell me|show|display)\b/i, // Find and summarize (with noun forms)
    /\b(read|find|look for|search for)\b.*\b(file|document|page)\b.*\b(summarize|summarise|summerize|summerise|summary|summaries|describe|explain|overview)\b/i // Find file and summarize (more flexible pattern with noun forms)
  ];
 ...
 * Waits for DOM mutations to settle and any navigation to complete
 * @param {number} timeout - Maximum time to wait in ms (default: 3000)
 * @param {number} stabilityTime - Time with no changes to consider stable (default: 500)
 * @returns {Promise<void>}
 */
async function waitForPageStable(timeout = 3000, stabilityTime = 500) {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab || !tab.id) {
      Logger.warn('[waitForPageStable] No active tab found');
      return;
    }
    ...
    if (tabStatus.status === 'loading') {
      Logger.info('[waitForPageStable] Page is loading, waiting for completion...');
    ...
    // Then wait for DOM to stabilize (no mutations for stabilityTime)
    Logger.info('[waitForPageStable] Waiting for DOM to stabilize...');
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (timeout, stabilityTime) => {
        return new Promise((resolve) => {
          ...
          const checkStability = setInterval(() => {
            ...
            if (timeSinceLastMutation >= stabilityTime && !resolved) {
              resolved = true;
              observer.disconnect();
      ...
      args: [timeout, stabilityTime]
    });

    Logger.info('[waitForPageStable] Page is stable');
  } catch (error) {
    Logger.warn('[waitForPageStable] Error waiting for page:', error);
    // Fallback to simple delay if scripting fails
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
}

/**
 * Get current page context (HTML and visible text)
 * @param {boolean} includeText - Whether to include visible text content (default: false for backwards compatibility)
 * @returns {Promise<string|Object>} - Page HTML string, or object with {html, text} if includeText=true
 */
async function getPageContext(includeText = false) {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tab || !tab.id) {
      Logger.error('[getPageContext] No active tab found');
      return includeText ? { html: '', text: '' } : '';
    }

    const results = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (shouldIncludeText) => {
    ...
    const result = results && results[0] ? results[0].result : (includeText ? { html: '', text: '' } : '');
    return result;
  } catch (error) {
...
export async function executeActionsSequentially(actions, onProgress = null, originalUserPrompt = '', enableAdaptivePlanning = true, abortSignal = null, onInteraction = null) {
  ...
  const results = [];
  let loginPromptShown = false; // Track if we've already asked about login
  ...
  while (i < currentActions.length) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        // Wait for page to stabilize after action (so LLM sees the changes)
        if (executionResult.success) {
          Logger.info(`[executeActionsSequentially] Waiting for page to stabilize after action...`);
      ...
      // Only check for login if onInteraction is provided AND we haven't already asked
      if (onInteraction && executionResult.success && !loginPromptShown) {
        ...
        try {
          ...
          // Get URL to check if we're on a login page
          const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
          const pageUrl = (tab?.url || '').toLowerCase();

          // Check if URL indicates a login page
          const isLoginUrl = pageUrl.includes('/login') ||
                            pageUrl.includes('/signin') ||
                            pageUrl.includes('/sign-in') ||
                            pageUrl.includes('/auth') ||
                            pageUrl.includes('/sso') ||
                            pageUrl.includes('login.') ||
                            pageUrl.includes('signin.');

          // Check if page has a visible password input field (actual login form)
          // Must have password input AND be on a login URL or have login form structure
          const hasPasswordField = lowerContext.includes('type="password"') ||
                                   lowerContext.includes("type='password'");

          // Only consider it a login page if:
          // 1. Has password field AND is on a login URL, OR
          // 2. Has password field AND has a login/signin form (not just nav links)
          const hasLoginForm = hasPasswordField && (
            isLoginUrl ||
            lowerContext.includes('<form') && (
              lowerContext.includes('login') ||
              lowerContext.includes('signin') ||
          ...
          if (hasLoginForm) {
            ...
            loginPromptShown = true; // Mark that we've shown the prompt

            // Get site name for context (reuse tab from above)
      ...
      // Small delay between actions (page stability already waited after successful actions)
  ...
  if (completedCount > 0 && originalUserPrompt) {
    ...
    try {
      // Get final page context after all actions (with text for better answer extraction)
      const finalContextData = await getPageContext(true); // Get both HTML and text
      const finalContext = finalContextData.html || finalContextData; // Backwards compatible
      const finalText = finalContextData.text || '';

      Logger.info(`[executeActionsSequentially] Final context length: HTML=${finalContext.length}, Text=${finalText.length}`);
      ...
      if (abortSignal && abortSignal.aborted) {
        ...
        // ANSWER EXTRACTION: If this was a research question, try to extract the answer
        // For research questions, we should attempt extraction even if goal verification says "not fully achieved"
        // because the file/page might be open but the LLM just needs to read and summarize it
        if (isResearchQuestion(originalUserPrompt)) {
          Logger.info('[executeActionsSequentially] Research question detected - extracting answer from page...');
          Logger.info(`[executeActionsSequentially] Goal achieved status: ${goalAchieved}`);
          Logger.info('[executeActionsSequentially] Attempting answer extraction regardless of goal verification...');

          // For answer extraction, prefer visible text content over HTML (cleaner, easier to read)
          const contextForExtraction = finalText || finalContext;
          Logger.info(`[executeActionsSequentially] Using ${finalText ? 'visible text' : 'HTML'} for answer extraction (${contextForExtraction.length} chars)`);
          ...
          try {
            ...
            if (answerResult.success) {
              ...
              // Override goal achieved if we successfully extracted an answer
              if (!goalAchieved && extractedAnswer) {
                Logger.info('[executeActionsSequentially] Overriding goalAchieved to true since answer was extracted');
                goalAchieved = true;
              }
            } else {
  ...
  if (success && goalAchieved) {
    ...
    // Append MCP tool results to the success message
    // MCP tool results are stored in the 'reason' field and contain valuable output data
    // that users need to see (e.g., file contents, directory listings, API responses)
    const mcpResults = results.filter(r => 
      r.status === 'success' && 
      r.reason && 
      r.reason.includes('MCP tool') && 
      r.reason.includes('result:')
    );
    
    // If we found MCP tool results, append them to the message
    if (mcpResults.length > 0) {
      Logger.info(`[executeActionsSequentially] Found ${mcpResults.length} MCP tool result(s) to display`);
      message += '\n\n**Results:**\n';
      
      // Iterate through each MCP result and format it for display
      mcpResults.forEach((r, index) => {
        // Extract the result value from the reason string (format: 'MCP tool "name" result: value')
        const resultMatch = r.reason.match(/MCP tool "[^"]*" result:\s*(.+)$/s);
        
        if (resultMatch && resultMatch[1]) {
          // Successfully parsed the result value
          const resultValue = resultMatch[1].trim();
          
          // Truncate very long results to prevent UI overflow issues
          // Common with large file listings or file contents
          const MAX_RESULT_LENGTH = 2000;
          if (resultValue.length > MAX_RESULT_LENGTH) {
            // Show truncated result with length indicator
            message += `${resultValue.substring(0, MAX_RESULT_LENGTH)}\n\n... (truncated, ${resultValue.length} total characters)`;
          } else {
            // Show full result
            message += resultValue;
          }
        } else {
          // Fallback: show the entire reason if parsing fails
          // This ensures users always see some output
          message += r.reason;
        }
        
        // Add separator between multiple results
        if (index < mcpResults.length - 1) {
          message += '\n\n---\n\n';
        }
      });
    }

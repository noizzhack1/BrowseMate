...
export async function executeActionsSequentially(actions, onProgress = null, originalUserPrompt = '', enableAdaptivePlanning = true) {
  ...
  // GOAL VERIFICATION: Check if the original user goal was actually achieved
  let goalAchieved = false;
  let goalVerificationMessage = '';

  if (completedCount > 0 && failedCount === 0 && originalUserPrompt) {
    Logger.info('[executeActionsSequentially] All actions completed. Verifying if goal was achieved...');

    try {
      // Get final page state
      const finalContext = await getPageContext();

      // Ask LLM to verify if the goal was achieved
      const verification = await llm.verifyGoalCall(finalContext, originalUserPrompt, results);
      Logger.info('[executeActionsSequentially] Goal verification result:', verification);

      goalAchieved = verification.achieved;
      goalVerificationMessage = verification.message;
    } catch (verificationError) {
      Logger.error('[executeActionsSequentially] Goal verification failed:', verificationError);
      // If verification fails, assume success based on action completion
      goalAchieved = true;
      goalVerificationMessage = 'Could not verify goal completion, but all actions succeeded.';
    }
  }

  const success = completedCount > 0 && failedCount === 0 && goalAchieved;
  const message = success
    ? `All ${completedCount} actions completed successfully`
    : `Completed ${completedCount}/${currentActions.length} actions (${failedCount} failed)`;

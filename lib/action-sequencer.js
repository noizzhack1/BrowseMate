  ...
  while (i < currentActions.length) {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        let executorResult;
        let currentMaxTokens = 1500; // Start conservative to leave room for input context
        const MAX_TOKENS_LIMIT = 4000; // Keep reasonable to avoid exceeding model's 8192 total token limit

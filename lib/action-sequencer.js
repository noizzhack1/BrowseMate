...
async function requestUserInputForFields(fields, onInteraction) {
  ...
  for (const field of fields) {
    // Use the actual field label/placeholder from HTML directly
    const fieldPrompt = field.placeholder
      ? `${field.label} (${field.placeholder}):`
      : `${field.label}:`;

    // Ask user for this field's value
    const userValue = await onInteraction(fieldPrompt);

    if (userValue && userValue.trim()) {
      userInputs[field.name || field.label] = userValue.trim();

      // Create fill action for this field
      fillActions.push({
        action: 'fill',
  ...
  // Add submit button click action
  const parser = new DOMParser();
  ...
  // Store all user inputs temporarily for the executor to use

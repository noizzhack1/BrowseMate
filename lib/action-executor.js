          ...
          const WebActions = {
            ...
            clickLink: (text, exact = false) => {
              ...
              const link = links.find(a => {
                ...
                const linkTitle = normalize(a.getAttribute('title'));
                const linkAria = normalize(a.getAttribute('aria-label'));

                if (exact) {
                  return linkText === target || linkTitle === target || linkAria === target;
                }

                return (linkText && linkText.includes(target)) ||
                       (linkTitle && linkTitle.includes(target)) ||
                       (linkAria && linkAria.includes(target));
              });
            ...
            drag: (sourceSelector, targetSelector) => {
              ...
              const findElement = (selector) => {
                ...
                console.log(`[drag] Checking ${iframes.length} iframes for selector: ${selector}`);
                for (const iframe of iframes) {
                  try {
                    const iframeDoc = iframe.contentDocument || iframe.contentWindow?.document;
                    if (iframeDoc) {
                      console.log(`[drag] Checking iframe: ${iframe.id || iframe.src || 'unnamed'}`);
                      element = iframeDoc.querySelector(selector);
                      if (element) {
                        console.log(`[drag] Found element in iframe: ${iframe.id || iframe.src}`);
                        return { element, doc: iframeDoc };
                      }
                    } else {
                      console.log(`[drag] Could not access iframe document: ${iframe.id || iframe.src || 'unnamed'} (likely cross-origin)`);
                    }
                  } catch (e) {
                    // Skip iframes we can't access due to cross-origin restrictions
                    console.log(`[drag] Error accessing iframe: ${iframe.id || iframe.src || 'unnamed'}: ${e.message}`);
                    continue;
                  }
                }
                return null;
              ...
              if (!sourceResult) {
                // Debug: List available IDs and iframes
                ...
                const iframes = Array.from(document.querySelectorAll('iframe')).map((iframe, idx) =>
                  `iframe[${idx}]: ${iframe.id || iframe.src || 'unnamed'}`
                );
                throw new Error(`Source element not found: ${sourceSelector}. Main doc has ${mainIds.length} IDs. Found ${iframes.length} iframes: ${iframes.join(', ')}. Check if elements are in a cross-origin iframe.`);
              }
              if (!targetResult) {
                const mainIds = Array.from(document.querySelectorAll('*')).filter(el => el.id).map(el => `#${el.id}`);
                const iframes = Array.from(document.querySelectorAll('iframe')).map((iframe, idx) =>
                ...
                throw new Error(`Target element not found: ${targetSelector}. Main doc has ${mainIds.length} IDs. Found ${iframes.length} iframes: ${iframes.join(', ')}. Check if elements are in a cross-origin iframe.`);
              }
              ...
              // Create a DataTransfer object and set the drag data
              const dataTransfer = new DataTransfer();
              // Set drag data with the source element's ID (common pattern for drag-and-drop)
              dataTransfer.effectAllowed = 'all';
              dataTransfer.setData('text/plain', source.id || sourceSelector);
              dataTransfer.setData('text/html', source.outerHTML);
            ...
            findSearchInput: (value) => {
              ...
              for (const strategy of strategies) {
                try {
                  ...
                  if (input) {
                    // Filter out GitHub feedback form (NOT the file finder)
                    if (input.id === 'feedback' && window.location.hostname.includes('github.com')) {
                      console.log('[findSearchInput] Skipping GitHub feedback form (#feedback) - not a search input');
                      continue; // Skip this input and try next strategy
                    }
...
async function executeAction(action) {
  ...
  try {
    ...
    // Alternate strategy list: actions where waiting for selector can unblock "not found"
    const selectorBasedActions = new Set(['click', 'fill', 'select', 'check', 'hover', 'submit', 'focus', 'clear', 'getText', 'getValue', 'scroll']);
    const textClickActions = new Set(['click', 'clickButton', 'clickLink']);

    const attemptTextClickFallback = async (tabId, text) => {
      try {
        const results = await chrome.scripting.executeScript({
          target: { tabId },
          world: 'MAIN',
          func: (targetText) => {
            ...
            const candidates = Array.from(document.querySelectorAll('button, a, [role="button"], input[type="button"], input[type="submit"], input[type="reset"], div, span, li'));
            ...
            for (const el of candidates) {
              ...
              const isPartial = label.includes(target);

              if (isExact || isPartial) {
                matches.push({ el, isExact });
              }
            }

            // Prefer exact match; otherwise first partial
            const match = matches.find(m => m.isExact) || matches[0];
            if (match) {
              match.el.scrollIntoView({ block: 'center', behavior: 'instant' });
              match.el.click();
              return { success: true, message: `Fallback text click fired for "${targetText}"` };
            }

            return { success: false, message: `Fallback text click could not find "${targetText}"` };
          },
          args: [text]
        });
        ...
        Logger.warn('[executeAction] Fallback text click failed:', fallbackError);
      }

      return { success: false };
    };

    const performActionAttempt = async (attempt, context = {}) => {
      const attemptLabel = context.useAlternateApproach ? ' (alternate)' : '';
      Logger.info(`[executeAction] Attempt ${attempt}${attemptLabel}: Executing ${action.name}...`);
      ...
      // If we're in alternate mode and this is a selector-based action, wait for element first
      const primaryTarget = paramValues[0];
      if (context.useAlternateApproach && selectorBasedActions.has(action.name) && typeof primaryTarget === 'string') {
        try {
          Logger.info(`[executeAction] Attempt ${attempt}: Waiting for selector before alternate retry: ${primaryTarget}`);
          await executeWebAction('waitForElement', [primaryTarget, 3000]);
        } catch (waitError) {
          Logger.warn(`[executeAction] Attempt ${attempt}: waitForElement (alternate) failed:`, waitError);
        }
      }

      // If alternate and text-based click still failing, try a broader text click sweep
      if (context.useAlternateApproach && textClickActions.has(action.name) && typeof primaryTarget === 'string') {
        const fallbackResult = await attemptTextClickFallback(tabId, primaryTarget);
        if (fallbackResult.success) {
          Logger.info(`[executeAction] Attempt ${attempt}: Alternate text click succeeded for "${primaryTarget}"`);
      ...
      // click, clickButton, clickLink are included because they may trigger JavaScript handlers that don't modify HTML
      const noChangeExpected = ['scroll', 'hover', 'navigate', 'reload', 'goBack', 'goForward', 'openNewTab', 'focus', 'pressKey', 'fill', 'clear', 'getText', 'getValue', 'translatePage', 'showFireworks', 'click', 'clickButton', 'clickLink'];
      ...
      } else if (context.useAlternateApproach) {
        Logger.info(`[executeAction] Attempt ${attempt}: Alternate approach returning success despite no DOM change`);
        return {
          success: true,
          message: execResult.message || 'Action executed with alternate approach'
        };
      } else {
    ...
    // Execute the action with a single attempt plus one alternate approach if needed
    Logger.info('[executeAction] Starting execution with single attempt and alternate fallback');
    const result = await executeWithRetry((attempt, context) => {
      const useAlternate = context?.useAlternateApproach === true;
      return performActionAttempt(attempt, { ...context, useAlternateApproach: useAlternate });
    }, {
      alternateApproach: (attempt, context) => performActionAttempt(attempt, { ...context, useAlternateApproach: true })
    });

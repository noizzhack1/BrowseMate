      ...
      func: (actionName, ...params) => {
        try {
          ...
          const WebActions = {
            ...
            clickYouTubeVideo: (position = 1) => {
              console.log('[clickYouTubeVideo] Looking for YouTube video at position:', position);

              // Multiple strategies to find YouTube videos
              const strategies = [
                // Strategy 1: Search results video renderer (most common for search results)
                () => {
                  const videoRenderers = document.querySelectorAll('ytd-video-renderer, ytd-grid-video-renderer, ytd-compact-video-renderer');
                  if (videoRenderers.length >= position) {
                    const renderer = videoRenderers[position - 1];
                    // Try to find the clickable link within the renderer
                    const titleLink = renderer.querySelector('a#video-title, a.yt-simple-endpoint.style-scope.ytd-video-renderer');
                    if (titleLink) return titleLink;
                    // Fallback: any link with /watch?v= in the href
                    const anyVideoLink = renderer.querySelector('a[href*="/watch?v="]');
                    if (anyVideoLink) return anyVideoLink;
                  }
                  return null;
                },
                // Strategy 2: Direct video title links
                () => {
                  const videoTitles = document.querySelectorAll('a#video-title');
                  if (videoTitles.length >= position) {
                    return videoTitles[position - 1];
                  }
                  return null;
                },
                // Strategy 3: Any video link with /watch?v=
                () => {
                  const videoLinks = document.querySelectorAll('a[href*="/watch?v="]');
                  if (videoLinks.length >= position) {
                    return videoLinks[position - 1];
                  }
                  return null;
                },
                // Strategy 4: Rich item renderer (for home page)
                () => {
                  const richItems = document.querySelectorAll('ytd-rich-item-renderer');
                  if (richItems.length >= position) {
                    const item = richItems[position - 1];
              ...
              for (const strategy of strategies) {
                try {
                  const videoLink = strategy();
                  if (videoLink) {
                    console.log('[clickYouTubeVideo] Found video link using strategy, clicking...');
                    console.log('[clickYouTubeVideo] Video title:', videoLink.textContent?.trim().substring(0, 100));
                    console.log('[clickYouTubeVideo] Video href:', videoLink.href);

                    // Scroll into view first
                    videoLink.scrollIntoView({ behavior: 'smooth', block: 'center' });

                    // Wait a tiny bit for scroll then click
                    setTimeout(() => {
                      videoLink.click();
                    }, 200);
                    ...
                    return {
                      success: true,
                      message: `Clicked YouTube video at position ${position}: ${videoLink.textContent?.trim().substring(0, 50) || 'Unknown title'}`
                    };
                  }
                } catch (e) {
                  console.log('[clickYouTubeVideo] Strategy failed:', e.message);
                  continue;
                }
              }

              // If we get here, no video was found
              throw new Error(`Could not find YouTube video at position ${position}. Found ${document.querySelectorAll('a[href*="/watch?v="]').length} total video links on page.`);
            },
            clickGoogleResult: (position = 1) => {
              console.log('[clickGoogleResult] Looking for Google search result at position:', position);

              // Multiple strategies to find Google search results
              const strategies = [
                // Strategy 1: Standard organic results (most common)
                () => {
                  // Google uses various selectors for search results
                  const resultLinks = document.querySelectorAll('div.g a[href]:not([href^="#"]):not([href*="google.com/search"])');
                  const validLinks = Array.from(resultLinks).filter(link => {
                    // Filter out navigation links, ads, etc.
                    const href = link.href;
                    return href.startsWith('http') &&
                           !href.includes('accounts.google.com') &&
                           !href.includes('support.google.com') &&
                           link.querySelector('h3'); // Must have a title heading
                  });

                  if (validLinks.length >= position) {
                    return validLinks[position - 1];
                  }
                  return null;
                },
                // Strategy 2: Find by h3 title links
                () => {
                  const titleLinks = document.querySelectorAll('h3 a[href]:not([href^="#"])');
                  const validLinks = Array.from(titleLinks).filter(link => {
                    const href = link.href;
                ...
                // Strategy 3: Modern Google selectors
                () => {
                  const results = document.querySelectorAll('[jsname] a[href^="http"]:not([href*="google.com/search"])');
                  const validLinks = Array.from(results).filter(link => {
                    const parent = link.closest('div.g, div[data-sokoban-container]');
                    return parent !== null;
                  });
                ...
                // Strategy 4: Fallback - any blue link that looks like a result
                () => {
                  const allLinks = document.querySelectorAll('a[href^="http"]:not([href*="google.com"])');
                  const resultLinks = Array.from(allLinks).filter(link => {
                    // Check if link has text and is visible
                  ...
                  if (resultLinks.length >= position) {
                    return resultLinks[position - 1];
                  }
                  return null;
                }
              ];
              ...
              for (const strategy of strategies) {
                try {
                  const resultLink = strategy();
                  if (resultLink) {
                    console.log('[clickGoogleResult] Found result link using strategy, clicking...');
                    console.log('[clickGoogleResult] Result title:', resultLink.textContent?.trim().substring(0, 100));
                    console.log('[clickGoogleResult] Result href:', resultLink.href);

                    // Scroll into view first
                    resultLink.scrollIntoView({ behavior: 'smooth', block: 'center' });

                    // Click after a short delay to ensure scroll completes
                    setTimeout(() => {
                      resultLink.click();
                    }, 200);
                    ...
                    return {
                      success: true,
                      message: `Clicked Google search result at position ${position}: ${resultLink.textContent?.trim().substring(0, 50) || 'Unknown title'}`
                    };
                  }
                } catch (e) {
                  console.log('[clickGoogleResult] Strategy failed:', e.message);
                  continue;
                }
              }

              // If we get here, no result was found
              const totalLinks = document.querySelectorAll('a[href^="http"]').length;
              throw new Error(`Could not find Google search result at position ${position}. Found ${totalLinks} total links on page. Make sure you're on a Google search results page.`);
            }
          };
...
async function executeAction(action) {
  ...
  try {
    ...
    const result = await executeWithRetry(async (attempt) => {
      ...
      const noChangeExpected = ['scroll', 'hover', 'navigate', 'reload', 'goBack', 'goForward', 'openNewTab', 'focus', 'pressKey', 'fill', 'clear', 'getText', 'getValue', 'uploadFile'];

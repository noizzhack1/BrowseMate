...
export class MemoryManager {
  ...
  async editMessage(index, content) {
    if (index < 0 || index >= this.messages.length) {
      Logger.warn(`[MemoryManager] Invalid index for editMessage: ${index}`);
      return;
    }
    this.messages[index].content = content;
    await this.save();
    Logger.info(`[MemoryManager] Edited message at index ${index}`);
  }

  /**
   * Remove all messages after a given index (conversation rewind)
   * Keeps messages from 0 to index (inclusive), removes everything after
   * @param {number} index - The index to rewind to (messages after this index will be removed)
   * @returns {Promise<void>}
   */
  async removeMessagesAfter(index) {
    if (index < 0 || index >= this.messages.length) {
      Logger.warn(`[MemoryManager] Invalid index for removeMessagesAfter: ${index}`);
      return;
    }
    const removedCount = this.messages.length - (index + 1);
    this.messages = this.messages.slice(0, index + 1);
    await this.save();
    Logger.info(`[MemoryManager] Removed ${removedCount} message(s) after index ${index}. Remaining: ${this.messages.length}`);
  }

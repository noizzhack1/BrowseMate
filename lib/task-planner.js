  ...
  try {
    ...
    if (plannerResult.intent === 'question') {
      ...
      for (let i = 0; i < plannerResult.toolCalls.length; i++) {
        ...
        // Parse arguments, defaulting to empty object for actions with no parameters
        let functionArgs = {};
        try {
          const argsString = toolCall.function.arguments;
          if (argsString && argsString.trim() && argsString.trim() !== 'undefined') {
            functionArgs = JSON.parse(argsString);
          }
        } catch (e) {
          Logger.warn(`[processRequest] Failed to parse arguments for ${functionName}, using empty object:`, e);
          functionArgs = {};
        }

 ...
 */
async function processRequest(context, prompt, onProgress = null, abortSignal = null, conversationHistory = [], onStreamChunk = null) {
=======
 ...
 */
async function processRequest(context, prompt, onProgress = null, abortSignal = null, conversationHistory = [], onInteraction = null) {
>>>>>>> user
  ...
  try {
    ...
    if (plannerResult.intent === 'question') {
      ...
      // Prepare context string for streaming
      let contextStr = '';
      if (typeof context === 'string') {
        contextStr = context;
      } else if (context && typeof context === 'object') {
      ...
      // If we have a simple answer from planner, use it; otherwise stream
      if (plannerResult.answer && plannerResult.answer.length < 200) {
        // Short answer, return directly
        Logger.info('[processRequest] Short answer, returning directly');
        return {
          type: 'answer',
      ...
      // Stream the answer with structured thinking
      const llm = getLLMClient();
      let fullAnswer = '';

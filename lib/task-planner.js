 ...
 * @param {boolean} isContinuation - If true, suppress thinking callbacks (message edit/resend)
 * @returns {Promise<{type: string, message: string, success: boolean}>}
 */
async function processRequest(context, prompt, onProgress = null, abortSignal = null, conversationHistory = [], _onInteraction = null, _onStreamChunk = null, onThinking = null, isContinuation = false) {
  ...
  Logger.info('[processRequest] isContinuation:', isContinuation);
  ...
  try {
    ...
    const agentCallbacks = {
      onThinking: (thought) => {
        ...
        // Suppress thinking display if this is a continuation (message edit)
        if (onThinking && !isContinuation) {
          onThinking(thought);
        }
      },

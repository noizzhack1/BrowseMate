  ...
  try {
    ...
    if (plannerResult.intent === 'question') {
      ...
      if (plannerResult.taskList && plannerResult.actions) {
        ...
        const completedCount = executionResult.results.filter(r => r.status === 'success').length;
        const failedCount = executionResult.results.filter(r => r.status === 'failed').length;
        const totalCount = executionResult.results.length;

        let summaryMessage = '';

        if (executionResult.success && executionResult.goalAchieved) {
          // All actions completed successfully AND goal achieved
          summaryMessage = `✅ **Goal achieved!**\n\nAll ${completedCount} actions were executed successfully.`;
          if (executionResult.goalVerificationMessage) {
            summaryMessage += `\n\n${executionResult.goalVerificationMessage}`;
          }
        } else if (executionResult.success && !executionResult.goalAchieved) {
          // Actions succeeded but goal not achieved
          summaryMessage = `⚠️ **Actions completed, but goal not fully achieved.**\n\n`;
          summaryMessage += `All ${completedCount} actions executed successfully, but:\n`;
          summaryMessage += `${executionResult.goalVerificationMessage || 'The final goal was not reached.'}\n\n`;
          summaryMessage += `**Actions completed:**\n`;
          executionResult.results.forEach(r => {
            summaryMessage += `• ${r.action.description}\n`;
          });
        } else if (completedCount > 0 && failedCount > 0) {
          // Partial success
          summaryMessage = `⚠️ **Task partially completed.**\n\n`;
          summaryMessage += `✅ ${completedCount} action${completedCount !== 1 ? 's' : ''} completed successfully\n`;
          summaryMessage += `❌ ${failedCount} action${failedCount !== 1 ? 's' : ''} failed\n\n`;

          // Show what succeeded
          const successfulActions = executionResult.results.filter(r => r.status === 'success');
          if (successfulActions.length > 0) {
            summaryMessage += `**What worked:**\n`;
            successfulActions.forEach(r => {
              summaryMessage += `• ${r.action.description}\n`;
            });
            summaryMessage += `\n`;
          }
          ...
          // Show what failed
          const failedActions = executionResult.results.filter(r => r.status === 'failed');
          if (failedActions.length > 0) {
            summaryMessage += `**What didn't work:**\n`;
            failedActions.forEach(r => {
              summaryMessage += `• ${r.action.description}`;
              if (r.reason) {
                summaryMessage += ` (${r.reason})`;
              }
              summaryMessage += `\n`;
            });
          }
          ...
          // All actions failed
          summaryMessage = `❌ **Task failed.**\n\n`;
          summaryMessage += `None of the ${totalCount} actions could be completed.\n\n`;
          summaryMessage += `**Issues encountered:**\n`;
          executionResult.results.forEach(r => {
            summaryMessage += `• ${r.action.description}`;
            if (r.reason) {
              summaryMessage += ` - ${r.reason}`;

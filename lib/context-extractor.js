...
function extractLabel(element, doc) {
  ...
  if (element.name && labels.length === 0) {
    ...
    labels.push(element.name.replace(/([A-Z])/g, ' $1').replace(/[_-]/g, ' ').trim());
  }
...
function isElementVisible(element) {
  ...
  if (element.offsetWidth === 0 && element.offsetHeight === 0) {
    // Allow elements that might be positioned off-screen but still interactive
    if (!element.getAttribute('tabindex')) return false;
  }
...
function extractButtons(doc, startId) {
  ...
  // Regular buttons
  const buttons = doc.querySelectorAll('button, input[type="button"], input[type="submit"], input[type="reset"], [role="button"]');
...
/**
 * Extract select dropdowns from the document
 * @param {Document} doc - Document reference
 * @param {number} startId - Starting ID
 * @returns {{elements: InteractiveElement[], nextId: number}}
 */
function extractSelects(doc, startId) {
  ...
  Logger.debug(`[extractSelects] Found ${selects.length} select elements`);
  ...
  for (const select of selects) {
    ...
    elements.push({
      ...
      type: 'select',
      label: label,
      selector: generateSelector(select, doc),
  ...
  Logger.info(`[extractSelects] Extracted ${elements.length} visible selects`);
  return { elements, nextId: id };
}
...
export function formatContextForLLM(elements, summary) {
  ...
  if (grouped.select.length > 0) {
    ...
    grouped.select.forEach(el => lines.push(`  ${formatElement(el)}`));
    lines.push('');
  }

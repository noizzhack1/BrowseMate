...
> **IMPORTANT: All code runs in the same context**

| Aspect | Detail |
|--------|--------|
| **Runtime** | Chrome Extension (Manifest V3) |
| **Code Context** | All parts (UI, Task A, Task B) run together in the same execution context |
| **Communication** | **Direct function calls** - NO message passing needed between components |
| **DOM Access** | **Full DOM access** available in ALL parts of the code |
| **Page Context** | Use existing `getPageContext()` function - returns `{url, title, text, html}` |
| **API Keys** | Stored in `chrome.storage.sync` |

### What This Means
- Task A can directly call Task B functions (no `chrome.runtime.sendMessage`)
- All modules can access `document`, `window`, and the full DOM
- The existing `getPageContext()` function provides page HTML - no need to pass context as parameters
- Code executed via EVAL runs in the page context with full DOM manipulation capabilities
...
/**
 * Main entry point
 * NOTE: Uses getPageContext() internally - no need to pass context
 * @param {{type, target, value?}} action - Action to perform
 * @returns {Promise<{success, message}>}
 */
 ...
 * NOTE: Fetches context via getPageContext() internally
 * @param {{type, target, value?}} action - Action descriptor
 * @returns {Promise<{code, explanation}>}
 */
async function callActionsLLM(action) {}

/**
 * Execute generated code
 * NOTE: Has full DOM access - runs in page context
 ...
 * NOTE: Uses existing getPageContext().html
